...
const partOne = (instructions: number[], debug: boolean) => {
    const input = [1];
    const simulation = new IntcodeSimulator(instructions.slice(0), input);
    const outputs = [];
...
const resultOne = (_: any, result: number) => {
    return `BOOST keycode is ${result}`;
};

const resultTwo = (_: any, result: number) => {
    return `The coordinates of the distress signal are ${result}`;
};
