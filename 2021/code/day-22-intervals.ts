...
const partTwo = (input: Command[], debug: boolean) => {
    ...
    for (const xsegment of xsegments) {
        if (xsegment.segments.length === 1) {
            if (xsegment.segments[0].action === "on") {
                totalLit += (xsegment.to - xsegment.from + 1) 
                        * (xsegment.segments[0].y.to - xsegment.segments[0].y.from + 1) 
                        * (xsegment.segments[0].z.to - xsegment.segments[0].z.from + 1);
            }
            continue;
        }
        ...
        for (const ysegment of ysegments) {
            if (ysegment.segments.length === 1) {
                if (ysegment.segments[0].action === "on") {
                    totalLit += ysegment.xdiff
                            * (ysegment.to - ysegment.from + 1) 
                            * (ysegment.segments[0].z.to - ysegment.segments[0].z.from + 1);
                }
                continue;
            }
            ...
            for (const zsegment of zsegments) {
                if (zsegment.segments.length === 1) {
                    if (zsegment.segments[0].action === "on") {
                        totalLit += zsegment.xdiff
                                * zsegment.ydiff
                                * (zsegment.to - zsegment.from + 1);
                    }
                    continue;
                }
